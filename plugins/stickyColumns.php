...
class stickyColumns
{
	...
	public function head()
	{
		...
		if($this->stickyColumnLeft!="")
			echo "#table tbody td[id$='[{$this->stickyColumnLeft}]']{position:sticky;left:0px;z-index:2;border-right: 1px solid grey;} ";
		//make right column sticky. Works only for data rows, not for headers
		if($this->stickyColumnRight!="")
			echo "#table tbody td[id$='[{$this->stickyColumnRight}]']{position:sticky;right:0px;z-index:2;border-left: 1px solid grey;} ";
		//make header sticky.
